    ...
    switch($proceso){
        case "NUEVO":
            try{
                ...
                $duplicado = $objVen->consultarVentaExistente($_POST['idtipocomprobante'],
                                            $_POST['serie'],$correlativo);
                ...
                if($duplicado->rowCount()==0 && count($problemasStock)==0){
                    ...
                    if($existe->rowCount()>0){
                        ...
                        $objClie->insertar($_POST['nombre'],$_POST['idtipodocumento'],$_POST['nrodocumento'],$_POST['direccion'],1);
                        $existe = $objClie->consultarClienteDocumento($_POST['nrodocumento']);
                        if($existe->rowCount()>0){
                            $cliente = $existe->fetch(PDO::FETCH_NAMED);
                            $idcliente = $cliente['idcliente'];
                    ...
                    $venta["idtipocomprobante"] = $_POST["idtipocomprobante"]; 
                    $venta["serie"]             = $_POST["serie"];             
                    ...
                    $venta["vencimiento"]       = $_POST["vencimiento"];       
                    $venta["guiaremision"]      = $_POST["guiaremision"];      
                    $venta["ordencompra"]       = $_POST["ordencompra"];
                    $venta["modalidad"]         = $_POST["modalidad"];
                    $venta["tipocontrato"]      = $_POST["tipocontrato"];
                    $venta["tiempocuotas"]      = $_POST["tiempocuotas"];
                    $venta["tiempoentrega"]     = $_POST["tiempoentrega"];       

                    $resultado = $objVen->insertar($venta);   
                    $objVen->actualizarCorrelativo($_POST['idtipocomprobante'], $_POST['serie'], $correlativo);

                    $venta = $objVen->consultarVentaExistente($_POST['idtipocomprobante'],
                    $_POST['serie'],$correlativo);                    
                    if($venta->rowCount()>0){
        ...
        case "ACTUALIZAR":            
            try{
                $duplicado = $objVen->consultarVentaExistente($_POST['idtipocomprobante'],
                $_POST['serie'],$_POST['correlativo'], $_POST['idventa']);
